...
another assumption is that a only 5 subjects will be entered no more or less than 5 subjects shouldnot be entered.
...
if validrollno(rollno)=false
  print "rollno is wrong"
  return
endif
 elseif validname(name)=false
   print "wrong name format"
   return 
endif
if duplicate(rollno)=false
   print "duplicate rollno detected"
   return
endif
for i=0 to 5
  if minormarks(minor)=false OR majormarks(major)=false
  print "wrong minor or major mark enter it again"
  return
endif
